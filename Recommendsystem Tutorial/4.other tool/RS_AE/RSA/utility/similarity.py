...
# improved pearson
def pearson_sp(x1, x2):
    common = set(x1.keys()) & set(x2.keys())  # 获取两用户共评item_id/两物品共评user_id
    if len(common) == 0:  # 如果两用户/物品 没有共评，则返回0
        return 0
    ratingList1 = []  # 用户/物品a 的评分列表
    ratingList2 = []  # 用户/物品b 的评分列表
    for i in common:
        ratingList1.append(x1[i])
        ratingList2.append(x2[i])
    if len(ratingList1) == 0 or len(ratingList2) == 0:  # 用户/物品a或b没有评过分
        return 0
    avg1 = sum(ratingList1) / len(ratingList1)  # a的平均分
    avg2 = sum(ratingList2) / len(ratingList2)  # b的平均分
    mult = 0.0
    sum1 = 0.0
    sum2 = 0.0
    for i in range(len(ratingList1)):
        mult += (ratingList1[i] - avg1) * (ratingList2[i] - avg2)  # mult = sum((r1 - u1)*(r2 - u2))
        sum1 += pow(ratingList1[i] - avg1, 2)  # s1 = sum( (r1-u1)^2 )
        sum2 += pow(ratingList2[i] - avg2, 2)  # s2 = sum( (r2-u2)^2 )
    if sum1 == 0 or sum2 == 0:  # 特殊情况，用户/物品 对/被 所有 物品/用户 都评了一样的分数？
        return 0
    return mult / (sqrt(sum1) * sqrt(sum2))  # 相似度
