...
class TrustWalker(MF):
    ...
    def __init__(self, k):
        super(TrustWalker, self).__init__(k)
        np.random.seed(0)
        self.tg = TrustGetter()
        # self.init_model()
    ...
    def single_random_walk(self, user=5, item=3, k=0):
        ...
        if self.rg.containsUserItem(user, item):  # judge whether user u rate on item i, if so, return the rating.
            ...
            stop_prob, max_item, p_j = self.get_stop_prob(user, item, k)  # p_j 选择item j的概率
    ...
    def get_stop_prob(self, user, item, k):
        ...
        param = sigmoid_2(k)  # 1/(1+e^(-0.5z))

        u_items = self.rg.user_rated_items(user)  # item_id list
        print(u_items)
        if len(u_items) == 0:
            return 0, 0, 0
