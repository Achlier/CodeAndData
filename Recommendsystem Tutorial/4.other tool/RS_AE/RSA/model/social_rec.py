    ...
    def train_model(self, k):
        ...
        while iteration < self.config.maxIter:
            ...
            for index, line in enumerate(self.rg.trainSet()):
                ...
                self.loss += error ** 2
                p, q = self.P[u], self.Q[i]  # 1*f vector

                followees = self.tg.get_followees(user)  # {u_to_1:w,u_to_2:w,...}
                zs = np.zeros(self.config.factor)  # 1*f zero vector
                for followee in followees:
                    if self.rg.containsUser(user) and self.rg.containsUser(followee):
                        vminus = len(self.tg.get_followers(followee))  # ~ d - (k) in-degree
                        uplus = len(self.tg.get_followees(user))  # ~ d + (i)   out-degree
                        import math
                        try:
                            weight = math.sqrt(vminus / (uplus + vminus + 0.0))  # cik *
                        except ZeroDivisionError:
                            weight = 1
                        zid = self.rg.user[followee]  # k index
                        z = self.Z[zid]  # Zk vector 1*f
