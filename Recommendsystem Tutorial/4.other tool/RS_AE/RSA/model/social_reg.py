        ...
        for u in self.rg.user:
            for f in self.tg.get_followees(u):
                if self.user_sim.contains(u, f):
                    continue
                sim = self.get_sim(u, f)  # 计算相似度
                self.user_sim.set(u, f, sim)  # 记录计算的相似度
    ...
    def train_model(self, k):
        ...
        while iteration < self.config.maxIter:
            ...
            for index, line in enumerate(self.rg.trainSet()):
                ...
                followees = self.tg.get_followees(user)  # 用户关注(好友)的 set {u_to:w,....}
                for followee in followees:
                    if self.rg.containsUser(followee):
                        s = self.user_sim[user][followee]  # 好友的相似度
                        uf = self.P[self.rg.user[followee]]  # 好友对应的latent vector
