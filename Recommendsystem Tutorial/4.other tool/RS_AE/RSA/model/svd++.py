...
class SVDPP(MF):
    ...
    def train_model(self, k):
        ...
        while iteration < self.config.maxIter:
            ...
            for index, line in enumerate(self.rg.trainSet()):
                ...
                u = self.rg.user[user]  # u index
                i = self.rg.item[item]  # i index
                error = rating - self.predict(user, item)
                self.loss += error ** 2

                p, q = self.P[u], self.Q[i]
                nu, sum_y = self.get_sum_y(user)  # number of nei_items, item implicit preference
    ...
    def predict(self, u, i):
        if self.rg.containsUser(u) and self.rg.containsItem(i):
            ...
            u = self.rg.user[u]
            i = self.rg.item[i]
            return self.Q[i].dot(self.P[u] + sum_y) + self.rg.globalMean + self.Bi[i] + self.Bu[u]  # 多了个y
        else:
            return self.rg.globalMean

    def get_sum_y(self, u):
        if u in self.SY:  # 有就直接返回
            return self.SY[u]
        u_items = self.rg.user_rated_items(u)  # item_id list
        nu = len(u_items)  # number of items
        sum_y = np.zeros(self.config.factor)  # factors vector
        for j in u_items:
            sum_y += self.Y[self.rg.item[j]]  # item implicit preference of (self.rg.item[j] =) item_j index
        sum_y /= (np.sqrt(nu))  # get mean
        self.SY[u] = [nu, sum_y]  # user_id : [ number of items, item implicit preference ]
        return nu, sum_y
